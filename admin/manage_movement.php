...
<?php
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT bc.status as bstatus,bc.id as bcid,b.*,c.model,c.brand,c.transmission_id,c.engine_id FROM borrowed bc inner join buku b on b.id = bc.booked_id inner join pelaminan c on c.id = b.car_id where bc.id=".$_GET['id'])->fetch_array();
	foreach($qry as $k =>$v){
		$$k = $v;
	}
}
...
<div class="container-fluid">
	<form action="" id="manage-movement">
		...
		<div class="row form-group">
			<div class="col-md-8">
				<label class="control-label">Penyewa</label>
				<select name="book_id" id="" class="custom-select select2">
				<option value=""></option>
					<?php 
						$book = $conn->query("SELECT * FROM buku where status = 2 and id not in (SELECT booked_id from borrowed_cars ) ".(isset($id) ? " or id = $id " : "" )." ");
		...
		<div class="row form-group">
			<div class="col-md-12">
				<label class="control-label">No Registrasi Dekorasi.</label>
		...
		<div class="row form-group">
			<div class="col-md-12">
				<label class="control-label">Model Dekorasi.</label>
