...
Class Action {
	...
	function save_transmission(){
		...
		$data .= ", description = '$description' ";
			if(empty($id)){
				$save = $this->db->query("INSERT INTO tipe_dekorasi set $data");
			}else{
				$save = $this->db->query("UPDATE tipe_dekorasi set $data where id = $id");
			}
		if($save)
			return 1;
	}
	function delete_transmission(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM tipe_dekorasi where id = ".$id);
		if($delete){
			return 1;
		}
	}
	function save_engine(){
		...
		$data = " name = '$name' ";
			if(empty($id)){
				$save = $this->db->query("INSERT INTO tipe_bajuadat set $data");
			}else{
				$save = $this->db->query("UPDATE tipe_bajuadat set $data where id = $id");
			}
		if($save)
			return 1;
	}
	function delete_engine(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM tipe_bajuadat where id = ".$id);
		if($delete){
			return 1;
		}
	}
	...
	function delete_car(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM pelaminan where id = ".$id);
		if($delete){
			return 1;
		}
	}
	function save_book(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO buku set ".$data);
		}else{
			$save = $this->db->query("UPDATE buku set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	function delete_book(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM buku where id = ".$id);
		if($delete){
			return 1;
		}
	}
	function get_booked_details(){
		extract($_POST);
		$qry = $this->db->query("SELECT b.*,c.brand, c.model FROM buku b inner join pelaminan c on c.id = b.car_id where b.id = $id ")->fetch_array();
		$data = array();
		foreach($qry as $k=>$v){
			if(!is_numeric($k))
			$data[$k]= $v;
	...
	function save_movement(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO borrowed set ".$data);
			if($save){
		...
		$data .= ", status = '$status' ";
			$save = $this->db->query("UPDATE borrowed set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	function delete_movement(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM borrowed where id = ".$id);
		if($delete){
			return 1;
		}
	}
